...
def api_send():
    ...
    redis_db.publish('general', u'%s¯%s¯%s' % (message_time, html.escape(author), html.escape(content)))
...
def chat_room(room_id):
    ...
    for room_no, room in enumerate(BASE_ROOMS):
        room['is_active'] = int(room_id) == int(room['id'])
        current_room = room
        BASE_ROOMS[room_no] = current_room
